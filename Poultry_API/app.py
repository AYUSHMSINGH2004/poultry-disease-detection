...
import matplotlib
matplotlib.use('Agg') # Forces matplotlib to run "headless" without a GUI
...
if not GOOGLE_API_KEY:
    raise RuntimeError("Please set the GEMINI_API_KEY environment variable before starting the API.")
...
# --- UPGRADED VETERINARY KNOWLEDGE BASE ---
disease_suggestions = {
...
def image_to_base64(pil_image):
    """Helper function to convert PIL images to Base64 strings for the frontend."""
    buffered = io.BytesIO()
...
async def analyze_poultry(image: UploadFile = File(...)):
    try:
        ...
        # --- 1. THE MULTIMODAL SECURITY GUARD ---
        security_prompt = """
        ...
        if "TRUE" not in security_response.text.strip().upper():
            return {"status": "error", "message": "Access Denied: Image does not contain a poultry bird, body part, or organ."}

        # --- 2. CUSTOM MEDICAL PIPELINE ---
        ...
        if p_sick < 0.5:
            ...
            predicted_index = np.argmax(doctor_prediction)
        ...
        x = classifier_input
        for layer in active_model.layers[active_model.layers.index(base_layer)+1:]: x = layer(x)
        ...
        # Convert superimposed numpy array back to PIL Image for Base64 encoding
        ...
        import traceback
        traceback.print_exc() # Still prints the red text to your terminal for debugging
        
        error_str = str(e).lower()
        # If the error is about quotas or rate limits, send a clean UI message!
        ...
        # For any other massive crash, throw the standard 500 error
